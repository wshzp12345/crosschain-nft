...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        ...
        address receiver )  public returns (bytes32 messageId) {
            // transfer the NFT to the contract to lock NFT
            wnft.transferFrom(msg.sender, address(this), tokenId);
            // burn the WNFT before send to ccip
            wnft.burn(tokenId);
            bytes memory data = abi.encode(tokenId, to);
            bytes32 _messageId = sendMessagePayLINK(chainSelector, receiver, data);
            return _messageId;
        }
